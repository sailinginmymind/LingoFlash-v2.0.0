...
const App = {
    ...
    refreshFolders() {
        ...
        if (listTitle) listTitle.innerText = `Parole di: ${this.state.cartellaAttiva} 📝`;
  ...
  mostraReportFinale() {
    const report = QuizManager.getReport();
    ...
    // Generiamo l'HTML per le parole sbagliate
   ...
   _resetQuizDisplay(autoStart = false) { // Aggiungiamo un parametro di default
    ...
    async handleAutocomplete(val) {
        ...
        const translated = await TranslationAPI.translate(
            val,
            this.ui.sourceLangSelect.value,
            this.ui.langSelect.value
        );
